...
dY.Year.prototype.metaOf = function (zonekey) { if (zonekey.constructor === Array) return this.schema[zonekey[0]][zonekey[1]]; return this.schema[Object.keys(this.schema)[0]][zonekey]; }; dY.Year.prototype.valuesOf = function (zonekey) { return this.ticks.map(function (d) { return d.valueOf(zonekey); }); }; dY.Year.prototype.dailySummary = function (dayCount = 1) {
	var slcs = []; var t = 0; while (t < this.ticks.length) { var timespan = dY.timeSpan.hoursOfYear(t, t + 24 * dayCount - 1); var data = dY.util.summarizeTicks(this.schema, ticks.slice(t, t + 24 * dayCount)); slcs.push(new dY.STick(timespan, data)); t += 24 * dayCount; }
	return slcs;
}; dY.hourOfDaySummary = function (schema, ticks) {
	var sortedTicks = {}; for (var t in ticks) { var h = ticks[t].hourOfDay(); if (!sortedTicks.hasOwnProperty(h)) sortedTicks[h] = []; sortedTicks[h].push(ticks[t]); }
	...
	return ret;
}; var dY = dY || {}; dY.dt = {}; dY.dt.year = 1970; dY.dt.minsPerYr = 525600; dY.dt.millsPerMin = 60000; dY.dt.millsToMins = 0.00001666666; dY.dt.dateStringToDate = function (str) {
	splt = str.split("  "); date = splt[0].trim().split("/"); month = parseInt(date[0]) - 1
	day = parseInt(date[1])
...
dY.dt.niceFormat = function (dt) {
	var mth = dY.dt.monthTable[dt.getUTCMonth()].shortname; var dat = dt.getUTCDate(); var hours = dt.getUTCHours(); var mins = dt.getUTCMinutes(); var pad = function (n) { return (n < 10) ? ("0" + n) : n; }
...
dY.parser.stringToZoneKey = function (str) {
	str = str.trim(); if (str.indexOf(":") == -1) { if (str == "Date/Time") return false; dY.report("dy: Improperly formatted string for zoneKey. No separator found for '" + str + "'"); return false; }
...
dY.parser.handleParseEPlusResults = function (results, callback) {
	dY.report("dy: Parsing EPlus Results File"); if (!dY.parser.handleParseErrors(results)) { dY.report("Parser failed. Quitting."); return false; }
	schema = {}; if (results.meta.fields.length > 0) {
	...
	ticks = []; results.data.forEach(function (row, n) {
		hourOfYear = dY.dt.dateToHourOfYear(dY.dt.dateStringToDate(row["Date/Time"])); timespan = dY.timeSpan.hourOfYear(hourOfYear); data = {}; for (var zon in schema) { data[zon] = {}; for (var key in schema[zon]) { value = row[dY.parser.zoneKeyToString(zon, key)]; data[zon][key] = value; } }
		ticks.push(new dY.Tick(timespan, data));
	}); schema = dY.util.summarizeTicks(schema, ticks); yr = new dY.Year(schema, ticks)
	if (typeof (callback) === 'undefined') { return yr; } else { callback(yr); }
}
dY.parser.handleParseEPWResults = function (head, results, callback) {
	dY.report("dy: Parsing EPW Weather File"); if (!dY.parser.handleParseErrors(results)) { dY.report("Parser failed. Quitting."); return false; }
	...
	ticks = []; results.data.forEach(function (row, n) {
		...
		hourOfYear = dY.dt.dateToHourOfYear(dY.dt.dateStringToDate(datestring)); timespan = dY.timeSpan.hourOfYear(hourOfYear); data = {}; data["EPW"] = {}; dY.parser.EPWKeyDefs.forEach(function (keyDef) { value = row[keyDef.col]; data["EPW"][keyDef.key] = value; }); ticks.push(new dY.Tick(timespan, data));
	}); schema = dY.util.summarizeTicks(schema, ticks); yr = new dY.Year(schema, ticks)
...
dY.parser.handleEPWHeader = function (yr, headString) {
	...
	yr.epwhead.periods = {}; var pcnt = parseInt(headTypExtrmPeriods[1]); for (var p = 2; p < pcnt * 4 + 2; p += 4) {
		var type = headTypExtrmPeriods[p + 1].toLowerCase(); if (!yr.epwhead.periods.hasOwnProperty(type)) yr.epwhead.periods[type] = []
	...
	yr.epwhead.ground = []; var gcnt = parseInt(headGroundTemp[1]); for (var g = 2; g < gcnt * 16 + 2; g += 16) {
		var gobj = {}; gobj.depth = parseFloat(headGroundTemp[g]); gobj.conductivity = parseFloat(headGroundTemp[g + 1]); gobj.density = parseFloat(headGroundTemp[g + 2]); gobj.specificHeat = parseFloat(headGroundTemp[g + 3]); gobj.monthlyTemperature = []; for (var m = 0; m < 12; m++) { gobj.monthlyTemperature.push(parseFloat(headGroundTemp[g + m + 4])); }
...
dY.solarGeom.solarGeomAtHour = function (loc, argA, argB) {
	var dayOfYear = argA; var hourOfDay = argB; if (typeof argB === "undefined") {
		if (typeof argA === "number") { var dayOfYear = Math.floor(argA / 24); var hourOfDay = argA % 24; }
...
dY.util.splitAtDiscontinuousHours = function (arr, key) {
	var bins = []; var bin = [arr[0]]; var idx = arr[0][key]; for (var n = 1; n < arr.length; n++) {
		if (arr[n][key] != idx + 1) { bin.domain = [bin[0][key], bin[bin.length - 1][key]]; bins.push(bin); bin = [arr[n]]; } else { bin.push(arr[n]); }
		idx = arr[n][key];
	}
...
dY.util.summarizeTicks = function (schema, ticks) {
	var summarySchema = {}
	var alls = []; for (var zon in schema) {
	summarySchema[zon] = {}
	...
	for (var t in ticks) { for (var zon in schema) { for (var key in schema[zon]) { alls[[zon, key]].push(ticks[t].data[zon][key]); } } }; for (var zon in schema) {
		for (var key in schema[zon]) {
			var allsorted = alls[[zon, key]].sort(function (a, b) { return a - b }); var len = allsorted.length; summarySchema[zon][key].min = allsorted[0]; summarySchema[zon][key].q1 = allsorted[Math.floor(len * .25) - 1]; summarySchema[zon][key].q2 = allsorted[Math.floor(len * .50) - 1]; summarySchema[zon][key].q3 = allsorted[Math.floor(len * .75) - 1]; summarySchema[zon][key].max = allsorted[len - 1]; summarySchema[zon][key].domain = [summarySchema[zon][key].min, summarySchema[zon][key].max]; summarySchema[zon][key].median = summarySchema[zon][key].q2; var sum = 0; for (var i = 0; i < allsorted.length; i++) { sum += allsorted[i]; }
